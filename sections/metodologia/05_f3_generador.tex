...
Esta fase se centra en la implementación del núcleo algorítmico de TecnoTime, abordando el problema de la generación automática de horarios como un desafío de satisfacción de restricciones (CSP). El desarrollo abarca desde el diseño del motor de inferencia hasta la integración de una interfaz de usuario reactiva, permitiendo combinar restricciones académicas ineludibles con heurísticas de optimización personalizadas y criterios de preferencia definidos por el estudiante.
...
Sin embargo, durante las pruebas en dispositivos móviles reales, este enfoque presenta limitaciones significativas. La complejidad NP-completa inherente al problema de coloración de grafos genera tiempos de ejecución exponenciales conforme aumenta el número de materias candidatas. Adicionalmente, la construcción explícita del grafo de conflictos y las estructuras auxiliares de búsqueda consumen memoria excesiva en terminales con recursos limitados, provocando excepciones de memoria insuficiente en dispositivos de gama media.

Por estas razones, se opta por reemplazar Graph Coloring con un algoritmo de búsqueda en profundidad (DFS) con poda basado en backtracking. Este algoritmo construye combinaciones incrementales, verificando restricciones duras (choques, series) en cada paso sin necesidad de construir el grafo completo en memoria. Las soluciones válidas se evalúan mediante una estrategia de puntuación inyectada (ScheduleEvaluationStrategy), permitiendo mantener únicamente las mejores alternativas.
...
La interfaz de usuario del generador de horarios se diseña siguiendo los principios de Material Design 3 y las mejores prácticas de UX móvil, priorizando la claridad en la configuración y la facilidad de comparación entre alternativas generadas. El flujo de interacción se estructura en dos pantallas principales: una de configuración donde el usuario selecciona materias y ajusta parámetros, y otra de resultados donde puede visualizar, comparar y aplicar las soluciones propuestas.
...
El generador de horarios cumple con el objetivo de producir $N=10$ combinaciones válidas con un TTS $\leq$ 3 min para cargas típicas. Asimismo, la regeneración parcial demuestra tiempos de respuesta casi instantáneos ($\leq$ 1 s por cambio), verificándose además la correcta aplicación de las estrategias de ponderación en el ranking de soluciones.
