  ...
  \bottomrule
  \end{tabularx}
  \captionof{table}{Resultados de pruebas: Notificaciones y sincronización.}\label{tab:qa_sync}
  ...
  Esta categoría se enfoca en la validación de los flujos de administración académica, abarcando la gestión de materias inscritas y la personalización de preferencias de docentes. Se evalúan exhaustivamente las operaciones de alta, modificación y baja de registros para asegurar una experiencia de usuario coherente y sin fricciones durante el ciclo de vida de la inscripción.
  
  Se verifica la correcta persistencia de los cambios en la base de datos local y la sincronización de las preferencias con el motor de generación de horarios:
  ...
  La ejecución de estos casos de prueba garantiza que las interacciones del usuario con los componentes de gestión académica mantengan la integridad referencial y visual en todo momento. Se presta especial atención a la transición entre pantallas y la persistencia inmediata de las preferencias, elementos fundamentales para que el motor de generación de horarios disponga de datos precisos y actualizados en cada consulta.
  ...
  \bottomrule
  \end{tabularx}
  \captionof{table}{Resultados de pruebas: Gestión de materias y docentes.}\label{tab:qa_materias}
  Fuente: Elaboración propia.
  \end{minipage}
  \end{center}
  ...
  La solidez de la capa de persistencia se evalúa mediante la verificación de la atomicidad en las operaciones complejas y la estabilidad de las relaciones de datos. Estos mecanismos de control aseguran que la información académica del estudiante se mantenga íntegra incluso ante fallos inesperados de la aplicación o del sistema operativo, garantizando una recuperación de estado confiable y consistente.
  ...
  \bottomrule
  \end{tabularx}
  \captionof{table}{Resultados de pruebas: Persistencia y base de datos.}\label{tab:qa_db}
  Fuente: Elaboración propia.
  \end{minipage}
  \end{center}
  ...
  Estas pruebas garantizan que la personalización del sistema, como el tema visual y la configuración del calendario, persista correctamente entre sesiones. El control sobre estos ajustes globales asegura la consistencia operativa y mejora la experiencia del usuario al evitar la reconfiguración manual de la herramienta.
  ...
  \bottomrule
  \end{tabularx}
  \captionof{table}{Resultados de pruebas: Configuración y preferencias.}\label{tab:qa_config}
  Fuente: Elaboración propia.
  \end{minipage}
  \end{center}
  
  Para resolver el problema de actualización de días visibles (CONF-023), se implementa un mecanismo de recomposición basado en un estado observable que fuerza la reconstrucción del componente de calendario ante cambios en la configuración. Este enfoque notifica a los elementos dependientes para garantizar que la interfaz refleje las nuevas preferencias de forma directa.
  ...
  \bottomrule
  \end{tabularx}
  \captionof{table}{Resultados de pruebas: Asistente inteligente Simon.}\label{tab:qa_simon}
  Fuente: Elaboración propia.
  \end{minipage}
  \end{center}
